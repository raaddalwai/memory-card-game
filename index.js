...
function resetGame(gridLength, gridWidth){
    ...
    for (let i = 0; i < gridWidth; i++) {
        ...
        for (let j = 0; j < gridLength; j++) {
            ...
            currUsableList = usableList[(i+j)%2]
...
function showCard(elem) {
    ...
    thisElem.querySelector(".cell-back").style.background = `url("${level1[figure]}") center/cover no-repeat`
...
function cellClick(event) {
        ...
        if(tempStore1==null){
            ...
            if(tempStore2==null){
                tempStore2 = thisElem
                if(figure == finalList[tempStore1.dataset.row][tempStore1.dataset.column]){
...
function incrementScore(){
    ...
    if(score==maxcorrects){
        ...
        stopTimer()
        document.querySelectorAll('.cell').forEach((elem)=>{ try{ elem.removeEventListener('click', cellClick) }catch(e){} })
        gameOver = true
        const timeTaken = timerDuration - Math.max(0, timerRemaining)
        // check and update high-score for this board
        const result = checkAndUpdateHighScore(currentGridLength, currentGridWidth, score, timeTaken)
...
function endGameByTimeout(){
    ...
    document.querySelectorAll('.cell').forEach((elem)=>{
        try{ elem.removeEventListener('click', cellClick) }catch(e){}
    })
    gameOver = true
    stopTimer()
    const timeTaken = timerDuration - Math.max(0, timerRemaining)
...
function saveHighScore(length, width, entry){
    try{ localStorage.setItem(getHighScoreKey(length, width), JSON.stringify(entry)) }catch(e){}
}
...
function checkAndUpdateHighScore(length, width, scoreVal, timeTaken){
    ...
    if(scoreVal > existing.score || (scoreVal === existing.score && timeTaken < existing.timeTaken)){
        saveHighScore(length, width, entry)
